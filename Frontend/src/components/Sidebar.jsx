...
const Sidebar = ({ selectedTab, setSelectedTab, openModal, isOpen = false, onClose = () => {} }) => {
  ...
  return (
    <aside
      className={`fixed md:static inset-y-0 left-0 z-40 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform duration-200 w-64 p-5 flex flex-col border-r`}
      style={{ backgroundColor: '#FFFFFF', borderColor: themeColors.textMuted + '26' }}
    >
      {/* Mobile close */}
          ...
          {tabs.map((item) => (
            <li key={item.tab}>
              <button
                ...
                style={{
                  backgroundColor: selectedTab === item.tab ? themeColors.primaryPurple + '22' : 'transparent',
