...
exports.getMyImages = asyncHandler(async (req, res, next) => {
  ...
  console.log('Query params:', { limit, offset, projectId, order });
  ...
  if (rows.length > 0) {
    console.log('Sample image:', rows[0]);
  }
...
exports.getMyVideos = asyncHandler(async (req, res, next) => {
  ...
  if (rows.length > 0) {
    console.log('Sample video:', rows[0]);
  }
...
// NOTE: Old getAssetByProjectId endpoint removed
// Use /api/v1/ar/check/:projectId and /api/v1/ar/generate/:projectId instead
// For getting project assets, use /api/v1/ar/projects/:id/assets
